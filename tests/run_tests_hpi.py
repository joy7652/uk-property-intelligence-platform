...
# ///
# MAGIC %md
# MAGIC # Tests: House Price Index
# MAGIC
# MAGIC Run all after any change to `databricks_src/silver/transforms/hpi.py` or to
...
# MAGIC
# MAGIC The index supplies the area names and codes above district level, so a change to its geography handling moves `dim_area` as well as the fact keyed on it.
# MAGIC
...
run("tests/test_gold_transforms/test_fact_area_month_hpi.py")
