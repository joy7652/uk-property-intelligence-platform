...
# ///
# MAGIC %md
# MAGIC # Tests: Bank of England base rate
# MAGIC
# MAGIC Run all after any change to `databricks_src/silver/transforms/boe_base_rate.py` or to
...
# MAGIC
# MAGIC The narrowest closure in the project. The rate series feeds the calendar and nothing else, so a change here cannot reach an area or a small-area measure.
# MAGIC
...
# MAGIC
# MAGIC One table reads this source, and it has a transform.

# COMMAND ----------

run("tests/test_gold_transforms/test_dim_date.py")
