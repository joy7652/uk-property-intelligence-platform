...
# ///
# MAGIC %md
# MAGIC # Tests: Private rents
# MAGIC
# MAGIC Run all after any change to `databricks_src/silver/transforms/ons.py` or to
...
# MAGIC
# MAGIC The rent series is the only source naming the eight Northern Irish rental market areas, which carry no published code. `dim_area` assigns them one, so a change to how those rows are read moves the dimension.
# MAGIC
...
# MAGIC
# MAGIC Two tables read this source.
# MAGIC
...
run("tests/test_gold_transforms/test_fact_area_month_rent.py")
